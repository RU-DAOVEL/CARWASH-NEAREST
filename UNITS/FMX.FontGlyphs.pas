...
uses
  System.Types, System.SysUtils, System.UITypes, System.Generics.Collections,
...
type
  ...
  public
    constructor Create(const AOrigin: TPoint;
      const AAdvance, AVertAdvance: Single; const AStyle: TFontGlyphStyles);
  ...
  const
    gsBitmap = TFontGlyphSetting.Bitmap deprecated
      'Use TFontGlyphSetting.Bitmap';
    gsPremultipliedAlpha = TFontGlyphSetting.PremultipliedAlpha deprecated
      'Use TFontGlyphSetting.PremultipliedAlpha';
  ...
  private
    class var FCurrentManager: TFontGlyphManager;
  ...
  public
    ...
    function GetGlyph(const Char: UCS4Char; const Font: TFont;
  ...
  private type
    TMethodDescriptor = record
      ...
      class function Create(AMethod: TCharacterHandleFunction;
        AOverrideValue: Boolean): TMethodDescriptor; static; inline;
    end;
  private
    class var FMethods: TList<TMethodDescriptor>;
  protected
    /// <summary>
  ...
  public
    ...
    /// </param>
    class procedure RegisterCharacterHandleMethod(const Method
      : TCharacterHandleFunction; const OverrideValue: Boolean = False); static;
    /// <summary>
    /// Unregister character handler from the chain
    /// </summary>
    class procedure UnRegisterCharacterHandleMethod(const Method
      : TCharacterHandleFunction); static;
  end;
...
function TFontGlyphManager.GetBaseline(const Font: TFont;
  const Scale: Single): Single;
begin
...
{$ENDIF}
    if FCurrentManager = nil then
      raise ETFontGlyphManagerException.Create
        ('No TFontGlyphManager implementation found');
  end;
...
class function TFontGlyphManager.TMethodDescriptor.Create
  (AMethod: TCharacterHandleFunction; AOverrideValue: Boolean)
  : TMethodDescriptor;
begin
...
function TFontGlyphManager.IsColorfulCharacter(const Character
  : UCS4Char): Boolean;
var
  I: Integer;
  Flag: Boolean;
...
class procedure TFontGlyphManager.RegisterCharacterHandleMethod
  (const Method: TCharacterHandleFunction; const OverrideValue: Boolean);
begin
  FMethods.Add(TMethodDescriptor.Create(Method, OverrideValue));
end;

class procedure TFontGlyphManager.UnRegisterCharacterHandleMethod
  (const Method: TCharacterHandleFunction);
var
  I: Integer;
begin
  for I := 0 to FMethods.Count - 1 do
...
begin
  AddEnumElementAliases(TypeInfo(TFontGlyphSetting),
    ['gsBitmap', 'gsPremultipliedAlpha', 'gsPath']);
end;
